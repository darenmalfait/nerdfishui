...
export const metadata = {
	...
	title:
		'Cacheable route or fetch with no caching (Cache-Control absent or no-store)',
	severity: 'medium',
...
const AUTH_RE =
	/\b(cookies\(\)|headers\(\)|getSession\(|getServerSession\(|currentUser\(|clerkClient|auth\(\)|verifyJWT|verifyToken|jwt\.verify|decode\(|Bearer\s|Authorization|supabase\.auth\.)/i
		...
		if (hasGetHandler) {
			...
			if (!hasCacheControl && !AUTH_RE.test(content)) {
				out.push({
					...
					line: 1,
					evidence:
						'GET handler with no Cache-Control / revalidate / auth signal',
...
function isApplicable(path) {
	return (
		/\/(route|index|page|layout)\.(ts|tsx|js|jsx)$/.test(path) ||
		/\/api\//.test(path)
	)
}
