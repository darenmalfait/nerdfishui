...
const CONFIG_RE =
	/export\s+const\s+config\s*=\s*\{[^}]*\b(isr|prerender|runtime|split|regions)\b/
...
export function scan({ files }) {
	...
	for (const { path, content } of files) {
		...
		if (
			PRERENDER_RE.test(content) ||
			SSR_RE.test(content) ||
			CONFIG_RE.test(content)
		)
			continue
		out.push({
			pattern: metadata.id,
			...
			line: 1,
			evidence:
				'No `prerender`, `ssr`, or `config = { isr | runtime | ... }` export found',
