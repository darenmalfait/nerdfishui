...
>(({className, ...props}, ref) => (
  <TabsPrimivite.Trigger
    className={cx(
      'inline-flex min-w-[100px] items-center justify-center rounded-[0.185rem] px-3 py-1.5 text-sm font-medium text-primary transition-all  disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-inverted/90 data-[state=active]:text-inverted data-[state=active]:shadow-sm',
      className,
    )}
    {...props}
    ref={ref}
...
>(({className, ...props}, ref) => (
  <TabsPrimivite.Content
    className={cx(
      'mt-2 rounded-md bg-primary border shadow-outline p-6',
      className,
    )}
    {...props}
    ref={ref}
